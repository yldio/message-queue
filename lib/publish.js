...
module.exports = function(adp) {
  function Publish(opts) {
    ...
    function onError(err) {
      err = types.isObject(err) ? stringify(err) : err;
    ...
    cli.validate = function(data, schema, next) {
      if (schema) {
        if (types.isObject(data)) {
          ...
          next(new VError('Expecting object not %s', typeof data));
        }
      } else {
      ...
      ws.publish = function(data, next) {
        ...
        cli.validate(data, schema, function(err, serializedData) {
          ...
          if (! Buffer.isBuffer(serializedData)) {
            serializedData = new Buffer(serializedData);
          }

          emitter.publish(name, serializedData, function() {
             next(undefined, {
              ack: true,
              ts: new Date(),
