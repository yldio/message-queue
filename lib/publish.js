  ...
  function Publish(opts) {
    ...
    emitter.channel = function createWriteStream(name, opts) {
      ...
      ws.publish = function(data, next) {
        if (emitter.closed) {
          return cbError(
            ut.MQError('Connection closed', 'adapter', data),
            next);
        }
        ...
        cli.validate(data, schema, function(err, serializedData) {
          if (err) {
            cbError(
              ut.MQError(err, 'validation', data), next);
          } else {
            // not for kafka-node
            var buff = 'kafka' === adp.name ?
              serializedData :
            ...
            if (!emitter.ready) {
              ...
              emitter.publish(name, buff, function(err, res) {
                if (err) {
                  cbError(err, next);
                } else {
                  var resp = {
