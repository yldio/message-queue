  ...
  function Publish(opts) {
    ...
    emitter.channel = function createWriteStream(name, opts) {
      ...
      ws.publish = function(data, next) {
        if (emitter.closed) {
          return execute(ut.mqError('Connection closed',
            'adapter',
            data), next);
        }
        ...
        if (serializedData.error) {
          return execute(ut.mqError(serializedData.error,
            'validation',
            data), next);
        }
        ...
        var buff = new Buffer(serializedData.value);
